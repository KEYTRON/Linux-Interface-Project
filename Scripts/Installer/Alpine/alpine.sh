...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget "https://raw.githubusercontent.com/htr-tech/Linux-Interface-Project/master/Rootfs/Alpine/${archurl}/alpine-minirootfs-3.11.2-${archurl}.tar.gz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
...
wget "https://raw.githubusercontent.com/htr-tech/Linux-Interface-Project/master/Scripts/Installer/Alpine/resolv.conf" -P alpine-fs/etc
